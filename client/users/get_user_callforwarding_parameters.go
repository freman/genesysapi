// Code generated by go-swagger; DO NOT EDIT.

package users

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"
	"net/http"
	"time"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/runtime"
	cr "github.com/go-openapi/runtime/client"
	"github.com/go-openapi/strfmt"
)

// NewGetUserCallforwardingParams creates a new GetUserCallforwardingParams object
// with the default values initialized.
func NewGetUserCallforwardingParams() *GetUserCallforwardingParams {
	var ()
	return &GetUserCallforwardingParams{

		timeout: cr.DefaultTimeout,
	}
}

// NewGetUserCallforwardingParamsWithTimeout creates a new GetUserCallforwardingParams object
// with the default values initialized, and the ability to set a timeout on a request
func NewGetUserCallforwardingParamsWithTimeout(timeout time.Duration) *GetUserCallforwardingParams {
	var ()
	return &GetUserCallforwardingParams{

		timeout: timeout,
	}
}

// NewGetUserCallforwardingParamsWithContext creates a new GetUserCallforwardingParams object
// with the default values initialized, and the ability to set a context for a request
func NewGetUserCallforwardingParamsWithContext(ctx context.Context) *GetUserCallforwardingParams {
	var ()
	return &GetUserCallforwardingParams{

		Context: ctx,
	}
}

// NewGetUserCallforwardingParamsWithHTTPClient creates a new GetUserCallforwardingParams object
// with the default values initialized, and the ability to set a custom HTTPClient for a request
func NewGetUserCallforwardingParamsWithHTTPClient(client *http.Client) *GetUserCallforwardingParams {
	var ()
	return &GetUserCallforwardingParams{
		HTTPClient: client,
	}
}

/*GetUserCallforwardingParams contains all the parameters to send to the API endpoint
for the get user callforwarding operation typically these are written to a http.Request
*/
type GetUserCallforwardingParams struct {

	/*UserID
	  User ID

	*/
	UserID string

	timeout    time.Duration
	Context    context.Context
	HTTPClient *http.Client
}

// WithTimeout adds the timeout to the get user callforwarding params
func (o *GetUserCallforwardingParams) WithTimeout(timeout time.Duration) *GetUserCallforwardingParams {
	o.SetTimeout(timeout)
	return o
}

// SetTimeout adds the timeout to the get user callforwarding params
func (o *GetUserCallforwardingParams) SetTimeout(timeout time.Duration) {
	o.timeout = timeout
}

// WithContext adds the context to the get user callforwarding params
func (o *GetUserCallforwardingParams) WithContext(ctx context.Context) *GetUserCallforwardingParams {
	o.SetContext(ctx)
	return o
}

// SetContext adds the context to the get user callforwarding params
func (o *GetUserCallforwardingParams) SetContext(ctx context.Context) {
	o.Context = ctx
}

// WithHTTPClient adds the HTTPClient to the get user callforwarding params
func (o *GetUserCallforwardingParams) WithHTTPClient(client *http.Client) *GetUserCallforwardingParams {
	o.SetHTTPClient(client)
	return o
}

// SetHTTPClient adds the HTTPClient to the get user callforwarding params
func (o *GetUserCallforwardingParams) SetHTTPClient(client *http.Client) {
	o.HTTPClient = client
}

// WithUserID adds the userID to the get user callforwarding params
func (o *GetUserCallforwardingParams) WithUserID(userID string) *GetUserCallforwardingParams {
	o.SetUserID(userID)
	return o
}

// SetUserID adds the userId to the get user callforwarding params
func (o *GetUserCallforwardingParams) SetUserID(userID string) {
	o.UserID = userID
}

// WriteToRequest writes these params to a swagger request
func (o *GetUserCallforwardingParams) WriteToRequest(r runtime.ClientRequest, reg strfmt.Registry) error {

	if err := r.SetTimeout(o.timeout); err != nil {
		return err
	}
	var res []error

	// path param userId
	if err := r.SetPathParam("userId", o.UserID); err != nil {
		return err
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}
