		...
		if m.ReplyMessages[i] != nil {
			if err := m.ReplyMessages[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					...
					return ce.ValidateName("replyMessages" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
...
// ContextValidate validate this post text response based on the context it is used
func (m *PostTextResponse) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	var res []error

	if err := m.contextValidateReplyMessages(ctx, formats); err != nil {
		res = append(res, err)
	}
...
func (m *PostTextResponse) contextValidateReplyMessages(ctx context.Context, formats strfmt.Registry) error {

	for i := 0; i < len(m.ReplyMessages); i++ {

		if m.ReplyMessages[i] != nil {
			if err := m.ReplyMessages[i].ContextValidate(ctx, formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("replyMessages" + "." + strconv.Itoa(i))
				} else if ce, ok := err.(*errors.CompositeError); ok {
					return ce.ValidateName("replyMessages" + "." + strconv.Itoa(i))
